...
(*dado um inteiro positivo n, testa se n é um número primo.*)
let primo (n:int) = 
        let
                acc = ref 0 in
...
(*Escrever uma função para somar os elementos de um arranjo de inteiros.*)
...
(*Escrever uma função para encontrar o maior elemento de um arranjo. Essa função não é definida para arranjos vazios.*)
...
(*Escrever uma função para buscar um elemento em um arranjo. Essa função deve retornar o índice da primeira posição onde o elemento se encontra, caso se encontra, e -1 caso contrário.*)
let busca_vetor v n =
...
(*Escrever uma função que, dado um arranjo a e um valor v do mesmo tipo dos
* elementos do arranjo a, constrói um novo arranjo cujos elementos são v na
* posição 0 e os elementos de a nas posição 1 até Array.length a.*)
...
(*dado um vetor de strings v retorna um vetor de inteiros v' onde cada posição
...
(*aplica uma funcao f 'a-> 'b aplicada no vetor v retorna v' que é resultado da aplicação de f em v*)
...
let map (f: 'a -> 'b) (v:'a array) =
        if v = [||] then [||] 
                ...
                saida;;
(*refaça os exercicios anteriores utlizando a função map*)
...
(*dado um vetor tipo string retorna a concatenação dos seus elementos na mesma
...
(*dado um vetor tipo string retorna a concatenação dos seus elementos separados
...
* função f do tipo 'b -> 'a -> 'b.Seja entao a seq s tal que s(0) = init e
* s(n+1)=f s(n) v.(n),ou seja s(n+1) é o resultado da aplicação de f a s(n) e
* v.(n), o elemento de v na posição..Defina uma função que, dados v,init e
* f,calcule s(m), onde m é o numero de elementos de v resolva as qstoes 4 a 8
* com essa função 
...
(*dado dois vetores de tipo float de mesmo tamanho retorna um terceiro que contem em cada posição o maior elemento entre os dois vetores *)
...
* uma função f do tipo 'a -> 'b -> 'c. Defina uma funcao que retorna o vetor w
* de valores do tipo 'c, de mesmo tamanho que v e 'v, tal que o valor na i-esima
* posição de w é igual a aplicação de f ao valor na i-esima posição de v e o
* valor na i-esima posição de v'.refaça o exercico 10 a 12*)
...
(*Escreva uma função que, dada um arranjo, ordena os elementos de forma
...
(*Escreva uma função que, dada um arranjo de números float, determina qual a maior
...
(*obs. se todos os elementos são negativos, retornar o valor 0.0.
Escreva uma função que, dada um arranjo de números float, determina qual o tamanho
da maior faixa crescente.
[| 2.1 ; -3.0 ; 2.2 ; 1.0 ; -0.1 ; 0.2 |] -> 2
obs. se os valores estão em ordem decrescente, retornar o valor 0.*)
